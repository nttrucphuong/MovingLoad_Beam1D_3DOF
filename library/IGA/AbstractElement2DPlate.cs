...
namespace DEMSoft.IGA
{
  ...
  /// </summary>
  public abstract class AbstractElement2DPlate : AbstractElement
  {
    protected Face face;
    protected GaussPoints[,] gps;

    public AbstractElement2DPlate(AbstractPatch2D patch, int id)
      : base(patch, id)
    {
      face = new Face(this);
    }
    ...
    public override void InitializeGaussPoint()
    {
      ...
      for (int i = 0; i < numberOfGaussPointOnEachDirection; i++)
      {
        ...
        for (int j = 0; j < numberOfGaussPointOnEachDirection; j++)
        {
          ...
          {
            double[,][,] gradBasis = basis.GetDerivativeBivariateBasisFunctions(xi1, xi2, 2);
            for (int k = 0; k < nen; k++)
            {
          ...
          DoubleMatrix ddNdX = MatrixFunctions.Inverse(jac2) * term2.Transpose();
          ...
          else if (Material is FGMStructureOneVariableMaterial)
          {
            ...
            ////gps[i, j].EModulus = (matE != null) ? matE.GetValueProperty(point[direction]) : 0;
            ////gps[i, j].nu = (matNu != null) ? matNu.GetValueProperty(point[direction]) : 0;
            ////gps[i, j].ThermalConductivity = (matK != null) ? matK.GetValueProperty(point[direction]) : 0;
            ...
            ////gps[i, j].coefficientsthermalexpansion = (matEpsilon != null) ? matEpsilon.GetValueProperty(point[direction]) : 0;
            MaterialProperty matE = Material.GetProperty(MaterialPropertyName.YoungModulus);
            MaterialProperty matNu = Material.GetProperty(MaterialPropertyName.PoissonRatio);
            MaterialProperty matK = Material.GetProperty(MaterialPropertyName.IsotropicThermalConductivity);
    ...
    public override DoubleMatrix GradBasisFunction(params double[] xi)
    {
      ...
      for (int i = 0; i < nen; i++)
      {
        ...
        dNdxi[i, 0] = gradBasis[patch.GetINC(ien, 0) - span1 + p, patch.GetINC(ien, 1) - span2 + q][1, 0];
        dNdxi[i, 1] = gradBasis[patch.GetINC(ien, 0) - span1 + p, patch.GetINC(ien, 1) - span2 + q][0, 1];
      }
      return dNdxi;
    }
    ...
    protected DoubleMatrix Jacobian2At(DoubleMatrix ddNdxi)
    {
      ...
      int nen = patch.GetCountLocalBasisFunctions();
      int d = patch.GetCountDimension();
      DoubleMatrix dxdxi = new DoubleMatrix(3, 2);
      var cps = patch.GetSurface().ControlPoints;
      for (int j = 0; j < 2; j++)
      {
        for (int k = 0; k < nen; k++)
        {
          ...
          for (int i = 0; i < 3; i++)
          {
            dxdxi[i, j] += ddNdxi[k, j] * controlPointCoord[i];
          }
        }
      }
      return dxdxi;
    ...
    public override DoubleVector GetDisplacementLocal()
    {
      ...
      for (int i = 0; i < nen; i++)
      {
        ...
        U[d * i] = cps[patch.GetINC(ien, 0), patch.GetINC(ien, 1)].GetResult(Result.UX);
        U[d * i + 1] = cps[patch.GetINC(ien, 0), patch.GetINC(ien, 1)].GetResult(Result.UY);
        U[d * i + 2] = cps[patch.GetINC(ien, 0), patch.GetINC(ien, 1)].GetResult(Result.UZ);
        U[d * i + 3] = cps[patch.GetINC(ien, 0), patch.GetINC(ien, 1)].GetResult(Result.THETAX);
        U[d * i + 4] = cps[patch.GetINC(ien, 0), patch.GetINC(ien, 1)].GetResult(Result.THETAY);
      }
      return U;
    }
