...
{
  public class PatchStructure2D : AbstractPatch2D, IPatchStructure
  {
    public Structure2DState StateStress
    { get; set; }

    public PatchStructure2D(NURBSSurface surface, Structure2DState state)
    ...
    public double StrainAt(Result re, params double[] xi)
    {
      ...
      switch (re)
      {
        ...
        case Result.EPSILONZZ:
          disp = strain[2];
          if (StateStress == Structure2DState.PlaneStress)
          {
            double nu = elem.Material.GetProperty(MaterialPropertyName.PoissonRatio).GetValueProperty();
