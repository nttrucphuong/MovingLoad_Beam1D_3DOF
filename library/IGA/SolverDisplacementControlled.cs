...
namespace DEMSoft.IGA
{
  ...
  /// </summary>
  internal class SolverDisplacementControlled : AbstractNonlinearSolver
  {
    /// <summary>
    /// Constructor class
    /// </summary>
    ...
    public SolverDisplacementControlled(AbstractModelStructure model, TypeNonlinearSolver type)
      : base(model, type)
    {
    }
        ...
        if (nIter == 1)
        {
          ...
          switch (model.convergenceOption.criterionConvergence)
          {
            ...
            case CriterionConvergence.EnergyCriterion:
              ...
              DoubleVector deltaForce = internalForceGlobal - residualForce;
              switch (AbstractModel.TypeSchemeNonlinearSolver)
              {
                case TypeSchemeNonlinearSolver.Monolithic:
                case TypeSchemeNonlinearSolver.SingleStaggered:
                  ...
                  DoubleVector reducedu0 = MatrixTool.GetSubVector(du, noConstraintTArray0, AbstractModel.NumberOfCPUs);
                  DoubleVector reducedu1 = MatrixTool.GetSubVector(du, noConstraintTArray1, AbstractModel.NumberOfCPUs);
                  wnorm1 = Math.Abs(MatrixFunctions.Dot(reducedu0, MatrixTool.GetSubVector(deltaForce, noConstraintTArray0, AbstractModel.NumberOfCPUs)));
                  wnorm2 = Math.Abs(MatrixFunctions.Dot(reducedu1, MatrixTool.GetSubVector(deltaForce, noConstraintTArray1, AbstractModel.NumberOfCPUs)));
                  //wnorm = Math.Max(Math.Sqrt(MatrixFunctions.Dot(reducedu0, reducedu0)), Math.Sqrt(MatrixFunctions.Dot(reducedu1, reducedu1)));
                  break;
                case TypeSchemeNonlinearSolver.Staggered:
                  reduceDu = MatrixTool.GetSubVector(du, noConstraintTArray, AbstractModel.NumberOfCPUs);
                  wnorm = Math.Abs(MatrixFunctions.Dot(reduceDu, MatrixTool.GetSubVector(deltaForce, tArray, AbstractModel.NumberOfCPUs)));
                  break;
              }
