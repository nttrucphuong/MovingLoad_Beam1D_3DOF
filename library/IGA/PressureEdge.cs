...
namespace DEMSoft.IGA
{
  ...
  /// </summary>
  public class PressureEdge : Pressure
  {
    ...
    public PressureEdge(Edge e, bool isInGlobal, params FunctionRToR[] p)
    ...
    /// <param name="p">global coordinate (x-y) : p[0] = px, p[1] = py,  local coordinate (n-t) : p[0] = pn, p[1] = pt</param>
    public PressureEdge(Edge e, bool isInGlobal, bool isNaturalCoordinate, params FunctionRToR[] p)
    ...
    public override DoubleVector ComputeLocalLoadVector(double time = 1)
    {
      ...
      int dimension = e.GetFace().GetElement().GetPatch().GetCountDimension();
      int p = e.GetDegree(0);
      int n = dimension * (p + 1);
        ...
        for (int i = 0; i <= p; i++)
        {
          if (isInGlobal)//Load distribute on x-y direction
          {
            for (int j = 0; j < dimension; j++)
            {
              if (press[j] != null)
              {
                re[i * dimension + j] += a * w * J1 * J2 * Ni[i] * press[j].ValueAt(paraLoad);//x y
              }
            }
          }
          else//n-t direction
          {
            for (int j = 0; j < dimension; j++)
            {
              for (int jj = 0; jj < press.Length; jj++)
              {
                ...
                {
                  if (jj == 0)//Normal
                    re[i * dimension + j] += a * w * J1 * J2 * Ni[i] * press[jj].ValueAt(paraLoad) * unitNormalVector[j];//tao, eta
                  else//Tangent
                    re[i * dimension + j] += a * w * J1 * J2 * Ni[i] * press[jj].ValueAt(paraLoad) * unitTangentVector[j];//tao, eta
                }
              }
            }
          }
    ...
    {
      int d = 2;//GetMeshPart().GetCountDimension();
      Edge e = (Edge)GetMeshPart();
      int p = e.GetDegree(0);
      int n = p + 1;
