...
//      //  numElems[i] = (int)elem;
//      //}
//      //double[,] arrDof = new double[pp.Length, numElems.Length];
...
//      //  {
//      //    double err = Calculate(pp[i], numElems[j], out int dof, out double Cdis, out double Edis);
//      //    arrDof[i, j] = dof/*dof*//*Math.Log10(dof)*/;
//      //    arrError[i, j] = err/*err*/ /*Math.Log10(err);*/ ;
...
//          PairOfIsotropicElasticity.YoungModulusAndPoissonRatio, E, nuy)); //0.01
...
//      double[] realLength = new double[subData.Length];
...
//      double displacementMax = model.GetPatch(0).GetApproximateAt(Result.UY, 0.5);
//      errs = Math.Sqrt(1.0 / numPoint * errs);
//      Console.WriteLine(errs);
//      Plotter plotter = new Plotter();
//      PlotLine line = new PlotLine();
//      line.InputData(realLength, arrayData, "IGA");
//      line.SetColor(Color.Red);
//      line.SetLineWidth(3);
//      plotter.AddPlot(line);

//      PlotLine line_1 = new PlotLine();
//      line_1.InputData(realLength, subData, "Exact");
...
//        subData[i] = results[i, 1];
//        if (arrayData[i] != 0)
//          err += Math.Pow((subData[i] - arrayData[i]), 2); //sum((u-ui)^2)
//        sumExact += Math.Pow((subData[i]), 2); //sum(ui^2)
//      }
//      err = 1.0 / numPoint * Math.Sqrt(err) / Math.Sqrt(sumExact);
//      //err = Math.Sqrt(err); //Root Mean Square Err
//      dof = model.CountDOF();
